function [inc_U_v_opt,inc_U_ps_opt,inc_U_pc_opt,fval]=optimize_incentives(M_opt, U_v, U_ps, U_pc)
...
function [c, ceq] = blocking_pairs_constraints(incentives, M_opt, U_v, U_ps, U_pc)
    ...
    for l = 1:i
        for m = 1:j
            for n = 1:k
                if M_opt(l, m, n) == 0
                    ceq = [ceq; inc_U_v(l, m, n)];
                    ceq = [ceq; inc_U_ps(l, m, n)];
                    ceq = [ceq; inc_U_pc(l, m, n)];
                end
            end
        end
    end
            ...
            for n = 1:k
                if M_opt(l, m, n) == 0
                    for idx = 1:length(matched_i)
                        ...
                        if n == 1 && m == 1
                            % 乗客も貨物もない場合
                            c(end+1) = U_v(l, m, n) - (U_v(l, y1, z1) + inc_U_v(l, y1, z1));
                        elseif n == 1
                            % 貨物がない場合
                            c(end+1) = U_v(l, m, n) + U_ps(l, m, n) - (U_v(l, y1, z1) + inc_U_v(l, y1, z1) + U_ps(x2, m, z2) + inc_U_ps(x2, m, z2));
                        elseif m == 1
                            % 乗客がない場合
                            c(end+1) = U_v(l, m, n) + U_pc(l, m, n) - (U_v(l, y1, z1) + inc_U_v(l, y1, z1) + U_pc(x3, y3, n) + inc_U_pc(x3, y3, n));
                        else
                            % 乗客と貨物がいる場合
                            c(end+1) = U_v(l, m, n) + U_ps(l, m, n) + U_pc(l, m, n) - (U_v(l, y1, z1) + inc_U_v(l, y1, z1) + U_ps(x2, m, z2) + inc_U_ps(x2, m, z2) + U_pc(x3, y3, n) + inc_U_pc(x3, y3, n));
                        end
                    end
                end
            end
    ...
    % インセンティブの和が0になる制約
    ceq = [ceq; sum(inc_U_v(:)) + sum(inc_U_ps(:)) + sum(inc_U_pc(:))];
end
