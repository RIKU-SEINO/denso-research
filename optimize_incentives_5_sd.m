function [inc_U_v_opt, inc_U_ps_opt, inc_U_pc_opt, fval] = optimize_incentives_5_sd(M_opt, U_v, U_ps, U_pc,pre_inc)
    % インセンティブ変数の初期値
    [i, j, k] = size(M_opt);
    inc_U_v = zeros(i, j, k);
    ...
    % 目的関数
    % objective = @(incentives) sum(abs(incentives));
...
%     'TolFun', 1e-6, ...
%     'TolX', 1e-6, ...
%     'ConstraintTolerance', 1e-3);  % 制約の許容誤差を1e-3に設定
%                 % 解の収束許容誤差を設定
% 

    % 最適化の実行
    [incentives_opt, fval,exitflag] = fmincon(@(incentives) objective(incentives), initial_incentives, [], [], [], [], [], [], ...
        @(incentives) blocking_pairs_constraints(incentives, M_opt, U_v, U_ps, U_pc,pre_inc),options);
...
function [c, ceq] = blocking_pairs_constraints(incentives, M_opt, U_v, U_ps, U_pc,pre_inc)
    [i, j, k] = size(M_opt);
    ...
    % 制約数の計算
    num_constraints = i * j * k ;
    
    % 制約用配列の事前割り当て
    c = zeros(num_constraints, 1);
    ceq = zeros( numel(M_opt)*3 + 1 + num_constraints, 1);
    ceq_idx = 1;
    
    % マッチしていないプレイヤのインセンティブを0にする制約
    for l = 1:i
        for m = 1:j
            for n = 1:k
                if M_opt(l, m, n) == 0
        ...
        for m = 1:j
            for n = 1:k
                if M_opt(l, m, n) == 0
                    ...
                    elseif n ~= 1
                        % 貨物だけいる場合
                        c(c_idx) = U_pc(l, m, n) - (U_pc(x3, y3, n) + inc_U_pc(x3, y3, n));
                    end
                    c_idx = c_idx + 1;
                end
            end
    ...
    % インセンティブの和が0以下になる制約
    c(c_idx) = sum(inc_U_v(:)) + sum(inc_U_ps(:)) + sum(inc_U_pc(:)) + pre_inc;
