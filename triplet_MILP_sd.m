function [M_opt, u_max] = triplet_MILP_sd(U,U_adj,U_v,U_ps)
[i,j,k] = size(U);

% reshape
f = reshape(U,[],1);
f_adj = reshape(U_adj,[],1);
...
%[x,fval] = intlinprog(-f_adj,1:length(f),-A,b,Aeq,beq,zeros(length(f),1),[0;ones(length(f)-1,1)],options);
[x,fval] = intlinprog(-f_adj,1:length(f),[],[],Aeq,beq,zeros(length(f),1),[0;ones(length(f)-1,1)],options);
