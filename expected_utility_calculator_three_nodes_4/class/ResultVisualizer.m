  ...
  methods (Static)
    ...
    function display_state_values_as_bar(solutions, is_optimal)
      ...
      for i = 1:length(solutions)
        ...
        xticklabels(PlayerSet.simplified_labels_tex(all_possible_player_sets));
        xtickangle(90);
    ...
    function display_expected_utilities_as_bar(solutions, is_optimal)
      ...
      for policy_index = 1:length(solutions)
        ...
        for i = 1:size(x_evaluated, 1)
          ...
          % タイトルとラベルを設定
          title_str = sprintf('プレイヤ集合: %s', player_set.label());
          title(title_str);
          xticks(1:length(all_possible_players));
          xticklabels(Player.labels_tex(all_possible_players));
          xtickangle(90);
