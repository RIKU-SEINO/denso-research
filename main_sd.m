...
incentive_matrix = cell(n_iter,3);
total_n_ebp=0;
n_ebp=0;
total_mixed_ebp=0;
mixed_ebp=0;
...
n_no_inc=0;
R = 0;
total_inc_record=zeros(n_iter,1);
total_preinc_record=zeros(n_iter,1);
pre_inc_matrix=cell(n_iter,1);
pre_inc_num=zeros(n_iter,1);
sd_num=zeros(n_iter,1);
timeline_inc=zeros(n_iter,1);
...
for iter = 1:n_iter
    ...
    % Adjust U weights
    [adj_U_taxi,n_sd] = sd_inc(U_platform, U_taxi, PS_list);

    adj_U_platform=adj_U_taxi+U_ps+U_pc;

    sd_num(iter)=n_sd;
    % optimize the matching
    [M_opt, u_max] = triplet_MILP_sd(U_platform,adj_U_platform,adj_U_taxi,U_ps);
    ...
    pre_inc_matrix{iter}= M_opt.* adj_U_platform - M_opt.* U_platform;
    pre_inc_num(iter)=nnz(pre_inc_matrix{iter});

    U_taxi=U_taxi+pre_inc_matrix{iter};
    total_inc = sum(M_opt.* adj_U_platform,"all")-sum(M_opt.* U_platform,"all");

    total_preinc_record(iter)=total_inc;
%     %incentive
%     [inc_U_v_opt,inc_U_ps_opt,inc_U_pc_opt,incentives]=optimize_incentives_5_sd(M_opt,U_taxi,U_ps,U_pc,total_inc);
% 

%     total_inc_record(iter)=incentives;
%     timeline_inc(iter)=sum(total_preinc_record)+sum(total_inc_record);
%     incentive_matrix{iter,1} = inc_U_v_opt;
%     incentive_matrix{iter,2} = inc_U_ps_opt;
%     incentive_matrix{iter,3} = inc_U_pc_opt;
% 
