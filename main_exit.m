...
timeline_mixed_ratio = zeros(n_iter,3);
timeline_exit_ps=zeros(n_iter,1);
...
for iter = 1:n_iter
    ...
    %exit ps
    max_u_ps=zeros(length(V_list),1);
    PS_index_pop=[];
    for j=2:length(PS_list)
        for i = 2:length(V_list)
            
            [~,u_ps,~,~,~]=utility_triplet_with_type(V_list(i),PS_list(j),PC_list(1),iter);
            max_u_ps(i)=u_ps;
            
        end

        if max(max_u_ps)<=0
            PS_index_pop=[PS_index_pop,j];   
...
%     end
    ...
    PS_archive_0 = PS_archive(arrayfun(@(ps) ps.type == 0, PS_archive));
    PS_archive_1 = PS_archive(arrayfun(@(ps) ps.type == 1, PS_archive));

    ps0_is_mixed_list = arrayfun(@(ps) ps.is_mixed, PS_archive_0);
    ps1_is_mixed_list = arrayfun(@(ps) ps.is_mixed, PS_archive_1);
    pc_is_mixed_list = arrayfun(@(pc) pc.is_mixed, PC_archive);
    timeline_mixed_ratio(iter,:) = [sum(ps0_is_mixed_list)/length(PS_archive_0),sum(ps1_is_mixed_list)/length(PS_archive_1),sum(pc_is_mixed_list)/length(PC_archive)];
     disp(iter)
end
toc
