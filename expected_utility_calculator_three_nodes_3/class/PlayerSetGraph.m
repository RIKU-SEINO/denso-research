    ...
    function obj = generate_labels_and_graph(obj)
      ...
      for i = 1:length(all_possible_player_sets)
        ...
        for j = 1:length(all_possible_player_sets_after_transition)
          ...
          label_origin = sprintf('%s: %.2f', player_set.label, player_set.get_expected_utility_from_solution(obj.solution));
          label_after_transition = sprintf('%s: %.2f', player_set_after_transition.label, player_set_after_transition.get_expected_utility_from_solution(obj.solution));
        ...
        for j = 1:length(all_possible_player_matchings)
          ...
          label_before_matching = sprintf('%s: %.2f', player_set.label, player_set.get_expected_utility_from_solution(obj.solution));
          label_after_matching = sprintf('%s: %.2f', player_set_after_matching.label, player_set_after_matching.get_expected_utility_from_solution(obj.solution));
